...
def conv_i_in_list(cast_to, list_to_cast):
    """Converts each index in a list. Parameters are (str, list)"""
    if cast_to == 'bin':
...
def run():
    ...
    while True:
        ...
        if which_conv.startswith('Q'): break

        elif which_conv.startswith('M'):
